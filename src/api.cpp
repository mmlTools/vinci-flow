...
static HttpResult winhttp_get(const QUrl &url, const wchar_t *extraHeaders, DWORD maxBytes)
{
	...
	HINTERNET hSession = WinHttpOpen(L"obs-plugin/vinci-flow", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
	...
	HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"GET", to_wide(fullPath).c_str(), nullptr,
	...
	for (;;) {
		...
		if (total + avail > maxBytes) {
			out.error =
				QStringLiteral("Response exceeded size limit (%1 bytes).").arg((qulonglong)maxBytes);
			break;
		}
...
static void log_http_result(const char *kind, const QUrl &url, int status, const QByteArray &body, const QString &err,
			    quint64 winErr = 0)
{
...
void ApiClient::requestImage(const QString &imageUrl, int targetPx)
{
	...
	if (!diskPath.isEmpty()) {
		...
		if (f.exists() && f.open(QIODevice::ReadOnly)) {
			...
			if (!img.isNull()) {
				if (targetPx > 0)
					img = img.scaled(targetPx, targetPx, Qt::KeepAspectRatioByExpanding,
	...
	std::thread([this, url, u, targetPx, diskPath]() {
		...
		QMetaObject::invokeMethod(
			this,
			[this, u, targetPx, diskPath, status, body, err, url, winErr]() {
				m_imgInFlight.remove(u);
...
void ApiClient::fetchLowerThirds(bool force)
{
	...
	std::thread([this, url]() {
		...
		QMetaObject::invokeMethod(
			this,
			[this, status, body, err, url, winErr]() {
				m_fetchInFlight = false;

				// Always log failures and unexpected responses.
				if (!err.isEmpty() || status < 200 || status >= 300) {
					log_http_result("[api]", url, status, body,
							err.isEmpty() ? tr("HTTP %1").arg(status) : err, winErr);
				}
